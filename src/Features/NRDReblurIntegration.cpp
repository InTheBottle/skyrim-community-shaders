...
void NRDReblurIntegration::CreateSamplers()
{
	...
	DX::ThrowIfFailed(device->CreateSamplerState(&desc, m_samplers[0].put()));

	desc.Filter = D3D11_FILTER_MIN_MAG_MIP_LINEAR;
	DX::ThrowIfFailed(device->CreateSamplerState(&desc, m_samplers[1].put()));
}
