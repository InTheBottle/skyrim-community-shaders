...
namespace
{
	// Stands in for the upstream GpuPass.h helper, which this repo does not carry. Fans one
	// pass name to the internal profiler and to RenderDoc/PIX annotations, matching the
	// hand-written BeginPass/BeginPerfEvent pairing used elsewhere in the codebase.
	struct ScopedRuntimeUpscalerPass
	{
		explicit ScopedRuntimeUpscalerPass(const std::string& a_name)
		{
			if (globals::profiler) {
				globals::profiler->BeginPass(a_name);
				profilerActive = true;
			}

			auto state = globals::state;
			if (state && state->frameAnnotations) {
				state->BeginPerfEvent(a_name);
				annotationOpen = true;
			}
		}

		~ScopedRuntimeUpscalerPass()
		{
			if (annotationOpen)
		...
		ScopedRuntimeUpscalerPass(const ScopedRuntimeUpscalerPass&) = delete;
		ScopedRuntimeUpscalerPass& operator=(const ScopedRuntimeUpscalerPass&) = delete;
		ScopedRuntimeUpscalerPass(ScopedRuntimeUpscalerPass&&) = delete;
		ScopedRuntimeUpscalerPass& operator=(ScopedRuntimeUpscalerPass&&) = delete;
...
// One pass per enclosing scope, so a fixed identifier is sufficient here.
#define CS_GPU_PASS(name) const ScopedRuntimeUpscalerPass cs_gpu_pass_scope { name }
...
namespace
{
	...
	{
		// This repo has no VR path; FSR always runs as a single full-frame context.
		return false;
	}
...
{
	// Runs at DLL unload, where the device may already be gone -- a GPU-idle wait here
	// would stall teardown for the full bounded timeout with nothing left to wait on.
	ReleaseRuntimeUpscalerResourcesForRelatch(false);
	ResetFSRIdleFence();
}
...
bool FidelityFX::ShouldRequestRuntimeFsr4() const
{
	...
	return upscaling.fsr4RuntimeEnableBoot &&
	...
	float a_motionVectorScaleX, float a_motionVectorScaleY, float a_sharpness)
{
	...
	const std::string dispatchPassName = "Upscaling::RuntimeUpscalerDispatch";
	...
	try {
		...
		if (!copyIntoShared(a_color, runtimeColorShared[a_contextIndex], colorDesc.Width, colorDesc.Height, runtimeColorSharedDesc.Width, runtimeColorSharedDesc.Height) ||
			...
			// Menus produce no fresh motion vectors or depth; reset the history there
			// rather than accumulating against stale reprojection data.
	...
	float a_motionVectorScaleX, float a_motionVectorScaleY, float a_sharpness)
{
	...
	const uint32_t hostContextCount = 1u;
	...
	const std::string dispatchPassName = "Upscaling::HostFsr3Dispatch";
	CS_GPU_PASS(dispatchPassName);
	...
	if (runtimeFallbackReset)
		runtimeFallbackResetDispatchesRemaining--;
	dispatchParameters.reset = state->IsMainOrLoadingMenuOpen() || runtimeFallbackReset;
	dispatchParameters.preExposure = 1.0f;
	dispatchParameters.flags = 0;
