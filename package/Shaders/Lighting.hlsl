...
#	endif
	...
	float4 waterData = SharedData::GetWaterData(input.WorldPosition.xyz, 0);
...
#	if defined(VOLUMETRIC_SHADOWS)
	if (inWorld && !inReflection && ShadowSampling::HasDirectionalShadows())
		dirSoftShadow = ShadowSampling::GetLightingShadow(input.WorldPosition.xyz, 0, dirVSMDetailedShadow);
#	endif
...
#	if defined(SCREEN_SPACE_SHADOWS) && defined(DEFERRED)
	if (!SharedData::InInterior && dirLightAngle >= 0.0)
		dirDetailedShadow *= ScreenSpaceShadows::GetScreenSpaceShadow(input.Position.xyz, screenUV, screenNoise, 0);
#	endif
...
#		if defined(HAIR) && defined(CS_HAIR)
	if (SharedData::hairSpecularSettings.Enabled) {
		float hairShadow = Hair::HairSelfShadow(input.WorldPosition.xyz, DirLightDirection, screenNoise, 0);
...
#				if defined(HAIR) && defined(CS_HAIR)
		if (SharedData::hairSpecularSettings.Enabled) {
			float hairShadow = Hair::HairSelfShadow(input.WorldPosition.xyz, normalizedLightDirection, screenNoise, 0);
